...
`main.gd` (REQUIREMENTS.md Step 3) owns two responsibilities: the intro gate and spawning. `_ready()` pauses the tree (`get_tree().paused = true`) so nothing moves until the player has read the instructions, and connects to `IntroScreen`'s `start_requested` signal to unpause once it fires. It also connects to `seed_popped` on every direct child that has it (each `Seedling`, found by checking `has_signal("seed_popped")` rather than hand-wiring each instance's signal in the `.tscn`, so seedlings added later are covered automatically). The seed handler instantiates `seed.tscn`, sets its `plant_type` to the popped hybrid, positions it a small fixed offset beside the plant, and adds it as a child of `Main`. Step 6 will extend this same script with goal tracking and the win overlay.

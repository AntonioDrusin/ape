...
- `player.tscn` — the flying bug: thrust-based flight (not platformer walk/jump), proboscis-driven water sucking (button + proximity) and, away from water, firing (button, hold-to-spray) real `water_droplet.tscn` projectiles (pattern 7); still-present hover-based watering is removed in Step 4. Also pollen and seed carrying. All world interaction is via the pattern-2 sensor poll except fired droplets, which self-poll (pattern 7). Everything that mirrors when the bug turns is under `Visual`; collision and camera are deliberately outside it.
- `water_droplet.tscn` — a fired projectile, spawned by `main.gd` on the player's `water_fired` signal (pattern 1); integrates its own gravity and despawns via a fixed-lifetime `Timer` (pattern 7's active exception to pattern 2). No seedling/ground collision response yet (Step 4).

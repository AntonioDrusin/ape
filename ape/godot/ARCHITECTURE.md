...
- `flower_guard.tscn` — a second self-directed hazard alongside the gnat cloud (same active-entity shape as `enemy.gd`), meant to guard a specific flower against point-blank camping. Orbits its assigned `guarded_flower` on a fixed circle (REQUIREMENTS.md Step 1) and now builds/decays aggro from player dwell time near that flower, telegraphing a windup (tint, wing-buzz speed, rising-pitch `WindupBuzz`) that cancels if the player backs off (Step 2); the actual lunge/punishment lands in Step 3.

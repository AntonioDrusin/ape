...
- `player.tscn` — the flying bug: thrust-based flight (not platformer walk/jump), proboscis-driven water sucking (button + proximity) and hover-based watering, pollen and seed carrying — all world interaction via the pattern-2 sensor poll. Everything that mirrors when the bug turns is under `Visual`; collision and camera are deliberately outside it.

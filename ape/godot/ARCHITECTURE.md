...
- `flower_guard.tscn` — a second self-directed hazard alongside the gnat cloud (same active-entity shape as `enemy.gd`), meant to guard a specific flower against point-blank camping. Orbits its assigned `guarded_flower` on a fixed circle, building/decaying aggro from player dwell time near that flower and telegraphing a windup (tint, wing-buzz speed, rising-pitch `WindupBuzz`) that cancels if the player backs off (Steps 1-2). Once windup completes it dashes (`LUNGE` state) at the player's position captured at that instant — not homing — then eases back onto the orbit circle (`RETURN` state) before resuming patrol; the dash is not cancelable once started. Detects the player via `body_entered` (the root `Area2D`'s `monitoring=true` has been on since Step 1; the handler gates on `state == LUNGE` so incidental patrol/windup overlap never punishes) and, on contact, duck-types `steal_water()`/`lose_pollen()` exactly like `enemy.gd` (seeds untouched), playing a distinct `HitSound` (currently reusing `water_pour.wav` as a placeholder). Aggro always resets to 0 after the lunge resolves (hit or miss) and `reaggro_cooldown` blocks it from rebuilding for a beat afterward (Step 3). Placement/tuning pass and a dedicated lunge whoosh remain Steps 4-5.

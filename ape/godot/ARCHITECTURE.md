...
- Drinking: while resting on water (see above) with no active movement input, the bug is "landed on water" and drinks — `Proboscis` becomes visible and `water_level` fills from 0 to 1 over `water_fill_time` (4s), emitting `water_level_changed` for `hud.gd` to display. `DrinkSound` (a looping woosh) fades in alongside it.
...
- Watering: each physics tick, `player.gd` checks `WaterSensor`'s overlapping areas (same poll used for water landing, just filtered to the `seedling` group instead of `water`) for a `HoverZone`, resolving the hovered `Seedling` once per tick. If `water_level > 0` and a seedling is hovered, it calls `water(delta)` on the zone's parent `Seedling` (see `scenes/seedling.tscn` above), draining `water_level` from 1 to 0 over `water_drain_time` (2s) the same way drinking fills it, and re-emitting `water_level_changed`. `Proboscis`, `WaterDrip`, and the looping `WaterSound` woosh all reflect this state so the drip is visibly (and audibly) the source of the growth.
- Loop SFX fades: `DrinkSound`/`WaterSound` are continuous, not one-shot, so `player.gd`'s `_update_loop_sound()` ramps each one's `volume_db` toward on/off every physics tick (via `SFX_FADE_SPEED`) instead of hard play/stop, starting the stream on fade-in and stopping it once fully faded out — avoids the click of cutting a looping waveform mid-cycle.

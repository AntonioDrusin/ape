...
- `background.tscn` — `ParallaxBackground` (draws behind everything; `CanvasLayer` layer defaults to -100) with three `ParallaxLayer`s: `Sky` (screen-fixed `motion_scale` 0, a copper-style banded blue gradient of `Polygon2D` strips), `Clouds`, and `Trees` (soil/grass band plus `cloud.tscn` / `pine_tree.tscn` instances at varied position/scale, pattern 5). Instanced once as `Main`'s first child; scroll tracks the active `Camera2D` automatically, no wiring.

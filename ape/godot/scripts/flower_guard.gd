...
## A patrolling hazard guarding its own patch of the level against
## point-blank camping (see REQUIREMENTS.md). Step 1: deterministic
## circular patrol around its spawn position. Step 2: builds/decays aggro
## from player dwell time near that spawn position and telegraphs a
...
## unlike the gnat cloud's constant spin.
...
func _ready() -> void:
	_center = position
	_center_global = global_position
	_angle = 0.0
...
## Aggro tracks dwell time near the guard's own orbit center (its spawn
## position), not its current position on the circle -- this is what makes
## point-blank camping of the guard's territory (not just brushing past the
## guard mid-patrol) the thing that gets punished.
func _update_aggro(delta: float) -> void:
	...
	if not _player:
		aggro = maxf(aggro - aggro_decay_rate * delta, 0.0)
		return
	var distance := _center_global.distance_to(_player.global_position)
	...
	match state:
		State.PATROL:
			if aggro >= aggro_threshold:
				...
				windup_buzz.play()
		State.WINDUP:
			var distance := _center_global.distance_to(_player.global_position) if _player else INF
			if distance > notice_range:
